...
def str_label_to_number(label):
########### transform 4 characters labels to 4 integers
    CAPTCHA_map='0123456789abcdefghijklmnopqrstuvwxyz'
    num_label=np.zeros(4)
    for i in range(0,4):
        for j in range(0,36):
            if(label[i]==CAPTCHA_map[j]):
                num_label[i]=j                
    return num_label   
...
def main():
    ...
    names = {}
    str_label=[]
    digit_label=np.zeros((DATA_SIZE,4))
    

    for file in os.listdir('images'):
        os.remove(os.path.join('images', file))
########### generate training or testing dataset
    for i in range(DATA_SIZE):
        image, name = random_image_from_base(base, username)
        label = real_image_solution(username, name)
        num_label=str_label_to_number(label)
        str_label.append(label)
        digit_label[i,:]=num_label
        
        names[str(i)+'_'+label + '.jpg'] = label

        image.save(os.path.join('images', str(i)+'_'+label + '.jpg'))
    DL_label={'digit_label':digit_label,'str_label':str_label}
########### saving the labels to mat file    
    savemat('DL_label_'+str(DATA_SIZE),DL_label)
